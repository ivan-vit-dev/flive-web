...
import { WifiOff, Radio, LayoutDashboard, Users, ClipboardCopy } from "lucide-react";
...
// Keyboard shortcuts for common event types (single letter, lowercase)
const KEYBOARD_SHORTCUTS: Partial<Record<string, MatchEventType>> = {
  g: "goal",
  o: "own_goal",
  y: "yellow_card",
  r: "red_card",
...
// Reverse map: eventType → shortcut key label
const EVENT_SHORTCUT: Partial<Record<MatchEventType, string>> = Object.fromEntries(
  Object.entries(KEYBOARD_SHORTCUTS).map(([k, v]) => [v, k.toUpperCase()])
);
...
function generateMatchReport(match: Match, events: MatchEvent[], locale: string): string {
  const date = match.scheduledAt?.toDate?.();
  const dateStr = date
    ? new Intl.DateTimeFormat(locale, { dateStyle: "long", timeStyle: "short" }).format(date)
    : "";

  const goals = events.filter((e) => ["goal", "own_goal", "penalty_scored"].includes(e.type));
  const cards = events.filter((e) => ["yellow_card", "second_yellow", "red_card"].includes(e.type));

  const lines: string[] = [];
  lines.push("FLive Match Report");
  lines.push("=".repeat(40));
  lines.push(`${match.homeTeam} ${match.homeScore} - ${match.awayScore} ${match.awayTeam}`);
  ...
  if (goals.length > 0) {
    ...
    goals.forEach((g) => {
      ...
      const player = g.playerName ?? "Unknown";
      const og = g.type === "own_goal" ? " (og)" : "";
      lines.push(`  ${g.minute}' ${player} [${team}]${og}`);
    });
  }
  ...
  if (cards.length > 0) {
    ...
    lines.push("CARDS");
    cards.forEach((c) => {
      const team = c.team === "home" ? match.homeTeam : match.awayTeam;
      const player = c.playerName ?? "Unknown";
      const type = c.type === "yellow_card" ? "[Y]" : c.type === "second_yellow" ? "[Y/R]" : "[R]";
      lines.push(`  ${c.minute}' ${player} [${team}] ${type}`);
    });
  }

  lines.push("");
  lines.push(`Final: ${match.homeTeam} ${match.homeScore} - ${match.awayScore} ${match.awayTeam}`);
  if (match.homeShootoutScore > 0 || match.awayShootoutScore > 0) {
    lines.push(`Penalties: ${match.homeTeam} ${match.homeShootoutScore} - ${match.awayShootoutScore} ${match.awayTeam}`);
  }
  lines.push("");
  lines.push("Generated by FLive");
...
export function LiveControlPanel({ match, onMatchUpdated, onSecondsChange }: Props) {
  ...
  const [confirmingFinish, setConfirmingFinish] = useState<PhaseButton | null>(null);
  const [lastEvent, setLastEvent] = useState<{ id: string; data: Omit<MatchEvent, "id" | "createdAt" | "updatedAt"> } | null>(null);
  ...
  useEffect(() => {
    const onOnline = () => { setIsOnline(true); flushPendingEvents(); };
  ...
  // Keyboard shortcuts — only active when match is running
  useEffect(() => {
    if (!RUNNING_STATUSES.includes(match.status)) return;
    const handler = (e: KeyboardEvent) => {
      if (e.target instanceof HTMLInputElement || e.target instanceof HTMLTextAreaElement) return;
      if (e.metaKey || e.ctrlKey || e.altKey) return;
      const type = KEYBOARD_SHORTCUTS[e.key.toLowerCase()];
      if (!type || !match.enabledEventTypes.includes(type)) return;
      e.preventDefault();
      if (COMPLEX_EVENTS.includes(type)) {
        setSelectedEventType(type);
      } else {
        void handleSimpleEvent(type);
      }
    };
    window.addEventListener("keydown", handler);
    return () => window.removeEventListener("keydown", handler);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [match.status, match.enabledEventTypes]);
  ...
  const flushPendingEvents = useCallback(async () => {
    ...
    for (const pending of session.pendingEvents) {
      try { await addMatchEvent(match.id, pending.event); } catch { /* keep in queue */ }
    }
  ...
  const logEvent = async (eventData: Omit<MatchEvent, "id" | "createdAt" | "updatedAt">) => {
    if (isOnline) {
      try {
        const eventId = await addMatchEvent(match.id, eventData);
        onMatchUpdated();
        void loadEvents();

        // Store for undo
        setLastEvent({ id: eventId, data: eventData });
        const toastId = toast(
          (toastInstance) => (
            <div className="flex items-center gap-3">
              <span className="text-sm">{t("control.undoEventAdded")}</span>
              <button
                onClick={() => {
                  toast.dismiss(toastInstance.id);
                  void handleUndoEvent(eventId, eventData);
                }}
        ...
        setTimeout(() => {
          setLastEvent((prev) => (prev?.id === eventId ? null : prev));
        }, 5000);
        void toastId;
      } catch {
  ...
  const handleUndoEvent = useCallback(async (eventId: string, eventData: Omit<MatchEvent, "id" | "createdAt" | "updatedAt">) => {
    try {
      // Construct a minimal MatchEvent so deleteMatchEvent can adjust scores correctly
      const { Timestamp } = await import("firebase/firestore");
      await deleteMatchEvent(match.id, {
        id: eventId,
        ...eventData,
  ...
  const handlePhaseChange = async (btn: PhaseButton) => {
    const { nextStatus, eventType, nextPart } = btn;
    const isNewPart = (nextStatus === "live_second" || nextStatus === "live_part") && nextPart !== undefined && nextPart > 1;
  ...
  const handlePhaseButtonClick = (btn: PhaseButton) => {
    if (btn.isFinish) {
      setConfirmingFinish(btn);
    } else {
      void handlePhaseChange(btn);
    }
  };

  const handleExportReport = async () => {
    const report = generateMatchReport(match, events, locale);
    await navigator.clipboard.writeText(report);
    toast.success(t("control.reportCopied"));
  };
            ...
            <>
              ...
              {viewerCount > 0 && (
                <span className="mt-1 flex items-center gap-1 text-xs text-muted-foreground">
                  ...
                  {t("control.viewerCount", { count: viewerCount })}
                </span>
              )}
            </>
        ...
        <div className="flex gap-2 flex-wrap shrink-0 sm:order-1">
          {phaseButtons.map((btn) => (
            <Button
              ...
              onClick={() => handlePhaseButtonClick(btn)}
            >
              {btn.label}
            </Button>
          ))}
            ...
            return (
              <div key={group}>
                ...
                <div className="grid grid-cols-4 gap-2">
                  {visible.map((type) => {
                    const shortcut = EVENT_SHORTCUT[type];
                    return (
                      <Button
                        key={type}
                        variant="outline"
                        size="sm"
                        className="relative flex-col h-auto py-2 gap-1 w-full rounded-xl"
                        onClick={() =>
                      ...
                      >
                        {shortcut && RUNNING_STATUSES.includes(match.status) && (
                          <span className="absolute top-1 right-1.5 text-[9px] font-mono font-bold text-muted-foreground/60 leading-none">
                            {shortcut}
                          </span>
                        )}
      ...
      {match.status === "finished" && (
        <div className="border-t pt-4 flex flex-wrap gap-2">
          ...
          >
            ...
            {t("control.exportReport")}
          </button>
        </div>
      )}

      {/* Confirm finish dialog */}
      <Dialog open={!!confirmingFinish} onOpenChange={(open) => { if (!open) setConfirmingFinish(null); }}>
        <DialogContent showCloseButton={false}>
          <DialogHeader>
            <DialogTitle>{t("control.confirmFinishTitle")}</DialogTitle>
            <DialogDescription>{t("control.confirmFinishDesc")}</DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setConfirmingFinish(null)}>
              {t("common.cancel")}
            </Button>
            <Button
              variant="destructive"
              onClick={async () => {
                ...
                await handlePhaseChange(confirmingFinish);
              }}
            >
              {confirmingFinish?.label ?? t("control.finishMatch")}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
...
function getPhaseButtons(match: Match, t: ReturnType<typeof useTranslations>): PhaseButton[] {
  ...
  if (parts === 1) {
    switch (status) {
      ...
      case "live_part":
        return [{ label: t("control.finishMatch"), nextStatus: "finished", eventType: "match_finished", variant: "brand", nextPart: 1, isFinish: true }];
      default:
        return [];
    }
  }
      ...
      case "live_part":
        if (currentPart >= parts) {
          return [
            ...
            { label: t("control.finishMatch"), nextStatus: "finished", eventType: "match_finished", variant: "destructive", nextPart: currentPart, isFinish: true },
          ];
        }
        return [{ label: t("control.endPart", { n: currentPart }), nextStatus: "break", eventType: "part_end", variant: "outline", nextPart: currentPart }];
      case "break": {
        if (currentPart >= parts) {
          return [{ label: t("control.finishMatch"), nextStatus: "finished", eventType: "match_finished", variant: "brand", isFinish: true }];
        }
  ...
  switch (status) {
    ...
    case "half_time":
      if (currentPart >= 3) {
        return [
          { label: t("control.finishMatch"), nextStatus: "finished", eventType: "match_finished", variant: "outline", isFinish: true },
    ...
    case "live_second":
      return [
        ...
        { label: t("control.finishMatch"), nextStatus: "finished", eventType: "match_finished", variant: "destructive", nextPart: 2, isFinish: true },
      ];
    case "extra_time":
      return [
        { label: t("control.endExtraTime"), nextStatus: "half_time", eventType: "extra_time_end", variant: "outline", nextPart: 3 },
        { label: t("control.finishMatch"), nextStatus: "finished", eventType: "match_finished", variant: "destructive", isFinish: true },
      ];
